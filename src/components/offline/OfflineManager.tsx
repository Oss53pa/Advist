          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-4 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.connection')}</p>
            ...
            <div className="p-4 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.pending_actions')}</p>
                  <p className="text-lg font-semibold text-advist-gray900 dark:text-white">{pendingCount}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.cached_documents')}</p>
                  <p className="text-lg font-semibold text-advist-gray900 dark:text-white">{cachedDocuments}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.storage_used')}</p>
                  <p className="text-lg font-semibold text-advist-gray900 dark:text-white">{estimatedSize}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
              <div>
                <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.last_sync')}</p>
                <p className="font-medium text-advist-gray900 dark:text-white">{formatDate(lastSyncAt)}</p>
              </div>
              {lastSyncResult && (
                <div className="text-right">
                  <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.last_result')}</p>
                  <p className="font-medium">
                    <span className="text-advist-success">{lastSyncResult.synced} {t('offline.synced')}</span>
                    {lastSyncResult.failed > 0 && (
                      <span className="text-advist-error ml-2">{lastSyncResult.failed} {t('offline.failed')}</span>
                    )}
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {syncQueue.slice(0, 5).map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-advist-surface-dark dark:bg-advist-dark/50 rounded-lg">
                      <div className="flex items-center gap-2">
                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${
                          item.type === 'comment' ? 'bg-advist-gold-light text-advist-gray900 dark:bg-advist-dark/30 dark:text-advist-gold' :
                          item.type === 'validation' ? 'bg-green-50 text-advist-success dark:bg-green-50/30 dark:text-advist-success' :
            ...
            {pendingCount === 0 && (
              <div className="text-center py-4">
                <svg className="w-12 h-12 mx-auto text-advist-text-muted dark:text-advist-text-secondary mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p className="text-sm text-advist-text-secondary dark:text-advist-text-muted">{t('offline.all_synced')}</p>
              </div>
            )}
          </div>
        </CardContent>
