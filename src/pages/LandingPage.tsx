...
const HowItWorksSection: React.FC = () => {
  ...
  return (
    <section ref={ref} id="how-it-works" className="py-32 bg-[#0A0A0B]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 border border-[#C8A961]/20 rounded-full mb-6">
            ...
            <span className="text-sm font-medium text-[#C8A961]">Simple & rapide</span>
          </div>
        ...
        <div className="relative">
          ...
          <div className="absolute top-24 left-0 right-0 h-px bg-white/10 hidden md:block">
            <div
              className="h-full bg-[#C8A961] transition-all duration-500"
              ...
              >
                <div
                  className={`absolute -top-3 left-1/2 -translate-x-1/2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold z-10 transition-all duration-300 ${
                    i <= activeStep
                      ? 'bg-[#C8A961] text-[#0A0A0B] scale-110'
                ...
                <div
                  className={`relative p-6 bg-[#1A1A1D] rounded-2xl border-2 transition-all duration-300 ${
                    i === activeStep
                      ? 'border-[#C8A961]/50 shadow-xl shadow-[#C8A961]/5 scale-105'
                ...
                >
                  <div
                    className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 ${
                      i === activeStep ? 'bg-[#C8A961] scale-110' : 'bg-white/5'
                    }`}
                  >
                    <step.icon
                      className={`w-8 h-8 ${i === activeStep ? 'text-[#0A0A0B]' : 'text-white/40'}`}
                    />
                  </div>
...
const ComplianceSection: React.FC = () => {
  ...
  return (
    <section ref={ref} id="compliance" className="py-32 bg-[#0F0F11] relative overflow-hidden">
      <div className="relative max-w-7xl mx-auto px-6">
        <div
          className={`text-center mb-16 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 backdrop-blur border border-[#C8A961]/20 rounded-full mb-6">
            <BadgeCheck className="w-4 h-4 text-[#C8A961]" />
            <span className="text-sm font-medium text-[#C8A961]">Conformite & Integrations</span>
          </div>
        ...
        >
          {items.map((item, i) => (
            <div
              key={i}
              className="p-6 bg-white/5 border border-white/5 rounded-2xl hover:bg-white/[0.08] hover:border-[#C8A961]/10 transition-all"
            >
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 bg-[#C8A961]/10 rounded-2xl flex items-center justify-center flex-shrink-0">
                  <item.icon className="w-7 h-7 text-[#C8A961]" />
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.badges.map((badge, j) => (
                      <span
                        key={j}
                        className="px-3 py-1 bg-[#C8A961]/10 border border-[#C8A961]/20 rounded-full text-xs font-medium text-[#C8A961]/70"
                      >
                        {badge}
                      </span>
                    ))}
        ...
        >
          <div className="flex flex-wrap justify-center gap-4">
            {certifications.map((cert, i) => (
              ...
              >
                <BadgeCheck className="w-5 h-5 text-[#C8A961]/40" />
                <div>
...
const InteractiveDemoSection: React.FC<{ onOpenModal: () => void }> = ({ onOpenModal }) => {
  ...
  return (
    <section ref={ref} id="demo" className="py-32 bg-[#0A0A0B] relative overflow-hidden">
      <div className="relative max-w-7xl mx-auto px-6">
        <div
          className={`text-center mb-12 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 border border-[#C8A961]/20 rounded-full mb-6">
            <Play className="w-4 h-4 text-[#C8A961]" />
            <span className="text-sm font-medium text-[#C8A961]">Essayez maintenant</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-light text-white mb-4">
            Testez ADVIST en <span className="text-[#C8A961]">conditions reelles</span>
          </h2>
          <p className="text-lg text-white/40 max-w-2xl mx-auto">
            ...
            {demoTabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all ${
                  activeDemo === tab.id
                    ? 'bg-[#C8A961] text-[#0A0A0B] shadow-lg'
        ...
        >
          <div className="relative bg-[#1A1A1D] rounded-3xl shadow-2xl overflow-hidden border border-white/5">
            <div className="bg-[#0A0A0B] px-6 py-5">
              <div className="flex items-center justify-between">
                ...
                <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 rounded-xl">
                  <Sparkles className="w-4 h-4 text-[#C8A961]" />
                  <span className="text-[#C8A961]/60 text-sm font-medium">Demo interactive</span>
                </div>
              </div>
            </div>

            <div className="p-6 max-h-[500px] overflow-y-auto bg-[#1A1A1D]">
        ...
        <div className="text-center mt-12">
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={onOpenModal}
              className="group inline-flex items-center justify-center gap-3 px-7 py-3.5 bg-[#C8A961] text-[#0A0A0B] text-sm font-medium rounded-2xl hover:bg-[#D4B872] hover:shadow-xl transition-all"
            >
  ...
  return (
    <section ref={ref} className="py-32 bg-[#0A0A0B]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 border border-[#C8A961]/20 rounded-full mb-6">
            <Star className="w-4 h-4 text-[#C8A961]" />
            <span className="text-sm font-medium text-[#C8A961]">Temoignages</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-light text-white mb-4">
        ...
        >
          <div className="relative bg-[#1A1A1D] rounded-3xl p-8 md:p-12 border border-white/5">
            <Quote className="w-12 h-12 text-[#C8A961]/20 mb-6" />
            <p className="text-xl md:text-2xl text-white/70 font-medium leading-relaxed mb-8">
              "{testimonials[activeIndex].quote}"
            </p>
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-[#C8A961]/10 flex items-center justify-center text-[#C8A961] text-xl font-bold">
              ...
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${
                    i === activeIndex ? 'bg-[#C8A961] w-8' : 'bg-white/10 w-2 hover:bg-white/20'
                  }`}
                />
              ))}
            </div>
...
const IAPromotionSection: React.FC = () => {
  ...
  return (
    <section ref={ref} id="proph3t" className="py-32 bg-[#0F0F11]">
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#C8A961]/10 border border-[#C8A961]/20 rounded-full mb-6">
            <Sparkles className="w-4 h-4 text-[#C8A961]" />
            <span className="text-sm font-medium text-[#C8A961]">Nouveau - Option IA</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-light text-white mb-4">
            Decouvrez <span className="font-decorative text-[#C8A961]">Proph3t</span>
          </h2>
        ...
        <div
          className={`relative transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <div className="relative bg-[#1A1A1D] rounded-3xl border border-white/5 p-8 md:p-12">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              {/* Features */}
              <div>
                <div className="flex items-center gap-3 mb-8">
                  <div className="w-14 h-14 bg-[#C8A961] rounded-2xl flex items-center justify-center">
                    <Sparkles className="w-7 h-7 text-[#0A0A0B]" />
                  </div>
                  <div>
                    <h3 className="text-xl font-normal text-white">Offre IA</h3>
                    <p className="text-white/40">Disponible pour Business & Enterprise</p>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  {iaFeatures.map((feature, i) => (
                    <div key={i} className="group flex items-start gap-4">
                      <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-[#C8A961] transition-all duration-300">
                        <feature.icon className="w-6 h-6 text-white/40 group-hover:text-[#0A0A0B] transition-colors" />
                      </div>
                      <div>
              ...
              <div className="lg:pl-8 lg:border-l border-white/10">
                <div className="bg-[#0A0A0B] rounded-2xl p-8 border border-white/5">
                  ...
                  <ul className="space-y-3 mb-8">
                    {[
                      ...
                      <li key={i} className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-[#C8A961] rounded-full flex items-center justify-center flex-shrink-0">
