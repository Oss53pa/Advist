...
const ROLE_LABEL: Record<Role, { label: string; color: string; description: string }> = {
  ...
  app_admin: {
    label: 'Admin',
    color: '#3B82F6',
    description: "Gère l'équipe et les paramètres applicatifs",
  },
...
export default function TeamSettingsPage() {
  ...
  const pendingInvites = seats.filter(
    (s) => s.status === 'active' && !s.invitation_accepted_at
  ).length;
                ...
                <tr key={seat.id} style={{ opacity: seat.status === 'suspended' ? 0.5 : 1 }}>
                  ...
                  <td style={styles.td}>
                    {canIManage ? (
                      ...
                      <span
                        style={{ ...styles.roleBadge, background: ROLE_LABEL[seat.role].color }}
                      >
